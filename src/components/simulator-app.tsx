    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <Input
          ...
          className="pr-11 font-medium tabular-nums"
        />
        {suffix && (
...
function Summary() {
  const { result, hypotheses } = useSimulator();
  const microOk = result.revenue <= hypotheses.microCeiling;
  const cashOk = result.lowCash >= 0;
  return (
    <>
      <SectionTitle
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        <Metric
          ...
          detail={`${percent(result.realNet / result.revenue)} du chiffre d’affaires`}
          progress={(result.realNet / result.revenue) * 100}
        />
        <Metric
          label="Objectif 1 500 €"
          value={result.targetMonth ?? "Non atteint"}
          detail={
            result.targetMonth
              ? "Premier mois au-dessus de l’objectif"
              : "Ajustez le plan d’activité"
          }
        ...
        <Metric
          ...
          detail="Moyenne de juin à août 2027"
          progress={(result.cruiseNet / hypotheses.target) * 100}
        />
        <Metric
          label="Point bas"
          value={euro(result.lowCash)}
          detail={`${result.lowCashMonth} · apport ${cashOk ? "suffisant" : "insuffisant"}`}
          progress={cashOk ? 100 : 20}
          good={cashOk}
        />
        <Metric
          label="Occupation au pic"
          value={percent(result.maxOccupancy)}
          detail={`${Math.round(result.maxOccupancy * hypotheses.capacity)} h sur ${hypotheses.capacity} h`}
          progress={result.maxOccupancy * 100}
          good={result.maxOccupancy <= 0.9}
        />
        <Metric
          label="Statut micro / TVA"
          value={microOk ? "Micro OK" : "Plafond dépassé"}
          detail={`${euro(result.revenue)} de CA · ${result.revenue > hypotheses.vatCeiling ? "TVA à facturer" : "franchise TVA"}`}
          progress={(result.revenue / hypotheses.microCeiling) * 100}
          good={microOk}
        />
      </div>
            ...
            <div className="h-[390px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <ChartTooltip formatter={(v: number) => euro(v)} />
                  <Legend />
        ...
        <div className="space-y-3">
          <Card className="border-l-4 border-l-success">
            <CardContent className="p-5">
              <p className="text-xs font-bold uppercase text-success">Verdict</p>
              <p className="mt-2 font-display text-xl font-bold">Démarrage finançable</p>
              <p className="mt-2 text-sm text-muted-foreground">
                L’apport couvre le point bas. Le modèle atteint l’objectif en{" "}
                {result.targetMonth?.toLowerCase()}.
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Inclut 30 % de marge de sécurité.
              </p>
            </CardContent>
          </Card>
          {result.maxOccupancy > 0.85 && (
            <Card className="border-l-4 border-l-warning">
              <CardContent className="p-5">
                <p className="font-bold">Capacité sous tension</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  À plus de 85 %, proposer les nouveaux contrats entre 32 et 34 €/h.
                </p>
              </CardContent>
            </Card>
          )}
...
function HypothesesPage() {
  ...
  const n = (key: keyof Hypotheses) => (v: number) => setField(key, v as never);
  return (
    <>
      <SectionTitle
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <Button onClick={() => applyPreset(OFFICIAL)}>Officiel · prudent</Button>
        <Button variant="outline" onClick={() => applyPreset(FIELD_REALISTIC)}>
          Réaliste terrain
        </Button>
      </div>
      <div className="grid gap-4 xl:grid-cols-2">
        <FieldGroup title="Tarifs & durées">
          <NumberField
            ...
            value={h.hourlyB2B}
            onChange={n("hourlyB2B")}
            suffix="€/h"
          />
          <NumberField
            label="Part contrats annuels"
            value={h.annualShare * 100}
            onChange={(v) => setField("annualShare", v / 100)}
            suffix="%"
          />
          <NumberField
            label="Remise annuelle"
            value={h.annualDiscount * 100}
            onChange={(v) => setField("annualDiscount", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
            ...
            onChange={n("visitsPerWeek")}
          />
          <NumberField
            label="Heures / passage"
            value={h.hoursPerVisit}
            onChange={n("hoursPerVisit")}
            suffix="h"
            step={0.1}
          />
          <NumberField
            label="Vitrerie"
            value={h.glassRate}
            onChange={n("glassRate")}
            suffix="€/h"
          />
          <NumberField
            label="Rotation Airbnb"
            value={h.airbnbPrice}
            onChange={n("airbnbPrice")}
            suffix="€"
          />
          <NumberField
            label="Particuliers"
            value={h.privateRate}
            onChange={n("privateRate")}
            suffix="€/h"
          />
        </FieldGroup>
        <FieldGroup title="Prélèvements micro">
          <NumberField
            ...
            value={h.socialRate * 100}
            onChange={(v) => setField("socialRate", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
            label="Taux ACRE"
            value={h.acreRate * 100}
            onChange={(v) => setField("acreRate", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
            label="TMI"
            value={h.tmi * 100}
            onChange={(v) => setField("tmi", v / 100)}
            suffix="%"
          />
          <div className="flex items-center gap-3">
            <Switch checked={h.acre} onCheckedChange={(v) => setField("acre", v)} />
            <Label>Activer l’ACRE</Label>
          </div>
          <div className="flex items-center gap-3">
            <Switch checked={h.vfl} onCheckedChange={(v) => setField("vfl", v)} />
        ...
        <FieldGroup title="Charges">
          <NumberField
            label="Produits"
            value={h.productsRate * 100}
            onChange={(v) => setField("productsRate", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
            label="Déplacements"
            value={h.travelRate * 100}
            onChange={(v) => setField("travelRate", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
            label="Impayés B2B"
            value={h.unpaidRate * 100}
            onChange={(v) => setField("unpaidRate", v / 100)}
            suffix="%"
            step={0.1}
          />
          <NumberField
        ...
        <FieldGroup title="Capital, capacité & seuils">
          <NumberField
            ...
            onChange={n("contribution")}
            suffix="€"
          />
          <NumberField label="Matériel initial" value={h.capex} onChange={n("capex")} suffix="€" />
          <NumberField
            label="Encaissement à 30 jours"
            value={h.delayedShare * 100}
            onChange={(v) => setField("delayedShare", v / 100)}
            suffix="%"
          />
          <NumberField
          ...
          <table className="data-table min-w-[1050px]">
            ...
            <tbody>
                  ...
                  {h[key].map((v, i) => (
                    <td key={i}>
                      <Input
                        ...
                        value={v}
                        onChange={(e) => setMonthValue(key, i, Number(e.target.value))}
              ...
              <tr className="calculated">
                <th>Heures</th>
                {result.months.map((m) => (
                  <td key={m.month} className={m.overloaded ? "text-destructive font-bold" : ""}>
                    {Math.round(m.hours)} h {m.overloaded && "⚠"}
                  </td>
                ))}
              </tr>
            </tbody>
...
function Profit() {
  ...
  const totals = useMemo(
    () => ({
      b2b: result.months.reduce((s, m) => s + m.b2b, 0),
      glass: result.months.reduce((s, m) => s + m.glass, 0),
      airbnb: result.months.reduce((s, m) => s + m.airbnb, 0),
      private: result.months.reduce((s, m) => s + m.private, 0),
    }),
    [result],
  );
  const levy = result.months.reduce((s, m) => s + m.ca * m.rate, 0);
  const variable = result.revenue * (h.productsRate + h.travelRate);
  return (
    <>
      <SectionTitle
        eyebrow="Rentabilité"
        title="Compte de résultat analytique"
        text="Du chiffre d’affaires encaissé au net réel disponible après toutes les charges."
      />
      <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
            ...
            <table className="data-table">
              ...
              <tbody>
                {Object.entries(totals).map(([k, v]) => (
                  <tr key={k}>
                    <th className="capitalize">{k === "glass" ? "Vitrerie" : k}</th>
                    <td>{euro(v)}</td>
                    <td>{percent(v / result.revenue)}</td>
                  </tr>
                ))}
                <tr className="calculated">
                ...
                <tr>
                  <th>Prélèvements</th>
                  <td className="text-destructive">− {euro(levy)}</td>
                  <td>{percent(levy / result.revenue)}</td>
                </tr>
                <tr>
                  <th>Charges variables</th>
                  <td className="text-destructive">− {euro(variable)}</td>
                  <td>{percent(variable / result.revenue)}</td>
                </tr>
                <tr>
                  <th>Charges fixes</th>
                  <td className="text-destructive">
                    − {euro(h.fixedMonthly * 12 + h.renewalMonthly * 12)}
                  </td>
                  <td></td>
                </tr>
                <tr>
                ...
                <tr className="total">
                  ...
                  <td>{percent(result.realNet / result.revenue)}</td>
                </tr>
              </tbody>
            </table>
          </CardContent>
        ...
        <div className="space-y-3">
          ...
          <Metric
            label="Net de croisière"
            value={euro(result.cruiseNet)}
            detail="Moyenne juin–août"
            progress={(result.cruiseNet / h.target) * 100}
          />
        </div>
      </div>
    </>
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Les prestations B2B et vitrerie sont encaissées avec 30 jours de décalage."
      />
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={result.months}>
                ...
                <ChartTooltip formatter={(v: number) => euro(v)} />
                <ReferenceLine y={0} stroke="var(--destructive)" />
                <Area
                  dataKey="cash"
                  name="Trésorerie"
                  stroke="var(--primary)"
...
function Scenarios() {
  const { result, saved, saveScenario, loadScenario, deleteScenario } = useSimulator();
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Mesurez l’effet d’un choc de volume et de prix, puis sauvegardez vos propres hypothèses."
      />
                  ...
                  <Bar dataKey="net" name="Net réel" radius={[5, 5, 0, 0]}>
                    {result.scenarios.map((_, i) => (
                      <Cell
                        key={i}
                        fill={
                          i === 0 ? "var(--warning)" : i === 1 ? "var(--primary)" : "var(--success)"
                        }
                      />
                    ))}
                  </Bar>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 space-y-2">
              {saved.length === 0 && (
                <p className="text-sm text-muted-foreground">Aucun scénario enregistré.</p>
              )}
              {saved.map((s) => (
                <div key={s.name} className="flex items-center justify-between border-b py-2">
                  <Button variant="link" className="px-0" onClick={() => loadScenario(s)}>
                    {s.name}
                  </Button>
                  <Button
                    variant="ghost"
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Comparaison indicative à chiffre d’affaires identique. Les règles sont simplifiées pour aider à la décision."
      />
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {statuses.map((s, i) => (
          <Card key={s.name} className={i === 0 ? "border-primary" : ""}>
            <CardContent className="p-5">
              <div className="flex justify-between">
                <span className="grid size-7 place-content-center rounded-full bg-secondary text-xs font-bold">
                  {i + 1}
                </span>
                {i === 0 && <span className="badge-success">Recommandé</span>}
              </div>
              <h3 className="mt-6 font-display text-lg font-bold">{s.name}</h3>
              <p className="mt-2 text-2xl font-bold">{euro(s.value)}</p>
              <p className="text-xs text-muted-foreground">net en poche estimé / an</p>
      ...
      <Card className="mt-5">
        <CardContent className="grid gap-5 p-5 md:grid-cols-3">
          <div>
            <p className="font-bold">Micro-entreprise</p>
            <p className="mt-1 text-sm text-muted-foreground">
              À privilégier tant que les charges réelles restent faibles et que le plafond n’est pas
              atteint.
            </p>
          </div>
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Devient pertinente avec plus de charges, des investissements ou une embauche.
            </p>
          </div>
          <div>
            <p className="font-bold">SASU</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Souplesse de gouvernance, mais coût social supérieur pour une rémunération régulière.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Anticipez la TVA, le dépassement du régime micro et la croissance du revenu net."
      />
            ...
            <ResponsiveContainer>
              <ComposedChart data={result.projection}>
                ...
                <Bar
                  ...
                  radius={[4, 4, 0, 0]}
                />
                <Line dataKey="net" name="Net" stroke="var(--chart-net)" strokeWidth={3} />
                <ReferenceLine y={h.vatCeiling} stroke="var(--gold)" strokeDasharray="6 5" />
                <ReferenceLine
                  y={h.microCeiling}
...
const STEPS = [
  "Titre de séjour autorisant l’activité",
  "Récépissé valide",
  "Immatriculation INPI",
  "Attestation URSSAF",
  "RC Pro souscrite",
  "Compte bancaire dédié",
  "Matériel professionnel prêt",
...
function Startup() {
  const [checked, setChecked] = useState<boolean[]>(() => {
    ...
    try {
      return (
        JSON.parse(localStorage.getItem("az-startup-checklist") ?? "null") ?? STEPS.map(() => false)
      );
    } catch {
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Les formalités et conditions indispensables avant la première prestation."
      />
      <div className="grid gap-5 xl:grid-cols-[1fr_.7fr]">
        <Card>
          <CardContent className="p-5">
            ...
            <div className="mt-6 space-y-1">
              {STEPS.map((s, i) => (
                <label key={s} className="flex cursor-pointer items-center gap-3 border-b py-3">
                  <Checkbox checked={checked[i]} onCheckedChange={(v) => toggle(i, !!v)} />
        ...
        <div className="space-y-4">
          <Card className="border-t-2 border-t-gold">
            <CardContent className="p-5">
              ...
              <p className="mt-2 font-display text-3xl font-bold">785 €</p>
              <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                <p>Certificat de résidence · 300 €</p>
                <p>CPF / Certibiocide · 350 €</p>
                <p>Assurances & formalités · 135 €</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              <p className="font-bold">Critères de viabilité</p>
              <div className="mt-3 space-y-2 text-sm">
                {[
                  "Apport couvrant le point bas",
                  "Objectif net atteint avant 6 mois",
                  "Occupation sous 90 %",
...
const TERMS = [
  [
    "Micro-entreprise",
    "Régime simplifié : les cotisations sont calculées directement sur le chiffre d’affaires encaissé.",
  ],
  [
    "ACRE",
    "Réduction temporaire des cotisations sociales au démarrage. Ici, 15,9 % pendant les dix premiers mois.",
  ],
  [
    "VFL",
    "Versement fiscal libératoire : l’impôt est payé au fil du chiffre d’affaires, au taux de 1,7 %.",
  ],
  ["CFP", "Contribution à la formation professionnelle des artisans, fixée ici à 0,3 % du CA."],
  ["Franchise TVA", "En dessous de 37 500 €, aucune TVA n’est facturée aux clients."],
  [
    "Plafond micro",
    "Au-delà de 83 600 € de CA de services, il faut anticiper une sortie du régime.",
  ],
  ["Point bas", "Niveau de trésorerie le plus faible observé pendant la période."],
  [
    "Croisière",
    "Rythme représentatif une fois l’activité installée : moyenne des trois derniers mois.",
  ],
  [
    "BFR",
    "Besoin de fonds de roulement créé notamment par le décalage entre prestation et encaissement.",
  ],
  ["CAPEX", "Investissement initial durable, comme le matériel professionnel de 1 200 €."],
];
function Dictionary() {
  ...
  return (
    <>
      <SectionTitle
        ...
        text="Des définitions courtes, reliées aux chiffres de L’AZ du Clean."
      />
      <Input
...
export function SimulatorApp() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <aside
        className={`app-sidebar ${mobile ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        <div className="flex h-full flex-col">
          <div className="border-b border-sidebar-border p-4">
            <div className="flex items-center gap-3">
              <div className="grid size-11 shrink-0 place-content-center rounded-lg bg-sidebar-primary font-display text-lg font-black text-sidebar-primary-foreground">
                AZ
              </div>
              <div>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-background/95 px-4 backdrop-blur md:px-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="icon" onClick={exportCsv} aria-label="Exporter en CSV">
              <Download />
            </Button>
            <Button
              variant="outline"

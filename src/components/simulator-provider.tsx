...
function readInitial() {
  ...
  try {
    ...
    return hypothesesSchema.parse(
      hash ? decode(hash) : (JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "null") ?? OFFICIAL),
    );
  } catch {
    return OFFICIAL;
  }
...
export function SimulatorProvider({ children }: { children: ReactNode }) {
  ...
  const setField = useCallback(
    <K extends keyof Hypotheses>(key: K, value: Hypotheses[K]) =>
  ...
  const share = async () => {
    const url = `${window.location.origin}${window.location.pathname}#s=${encode(hypotheses)}`;
  ...
  const exportCsv = () => {
    ...
    const rows = result.months.map((m) =>
      [m.month, m.b2b, m.glass, m.airbnb, m.private, m.ca, m.net, m.hours, m.cash]
  ...
  return (
    <SimulatorContext.Provider
      value={{
        ...
        setField,
        setMonthValue,
        applyPreset: setHypotheses,
        saveScenario,
        loadScenario: (s) => setHypotheses(s.hypotheses),
        deleteScenario: (name) => setSaved((old) => old.filter((s) => s.name !== name)),
        share,
        exportCsv,
      }}
    >
